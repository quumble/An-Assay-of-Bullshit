...
PRIMARY_FEATURES = [
    ...
    "total_words",
    "mean_word_length",
    # Note: features.py serializer prefixes POS feature keys with "pos_" (see
    # analysis/features.py L587). The preregistration names them bare
    # (noun_ratio, etc.). The extractor's internal dict still uses the bare
    # names; only the serialized JSONL keys carry the prefix. We use the
    # on-disk names here so the lookup succeeds. See deviations.md.
    "pos_noun_ratio",
    "pos_verb_ratio",
    "pos_adj_ratio",
    "pos_adv_ratio",
...
def cosine_distance_matrix(vectors: list[list[float]]):
    ...
    sim = normed @ normed.T
    d = 1.0 - sim
    # Floating-point: sim values near 1 (e.g. on the diagonal) can produce
    # tiny negative distances (~1e-16). sklearn>=1.6's silhouette_score
    # rejects negative entries in a precomputed distance matrix. Clamp to
    # zero — this is a numerical-correctness fix, not a methods change.
