...
## Outputs and re-analysis

The original as-run analysis artifacts are in `Results and Analysis/`,
generated by the analyzer at the time of the run. They are preserved
unchanged as the as-run record.

A subsequent re-analysis with Claude Opus 4.7 assistance is in
`Claude_Analysis/`. It corrects a bug in the as-run analyzer that
silently dropped 5 of 13 preregistered Q2/Q3 features (a key-name
mismatch between the feature serializer and the analyzer's lookup
keys). It also includes a sensitivity rerun under strict-yes-only
validation scoring. See `Claude_Analysis/README.md` and
`Claude_Analysis/deviations.md` for details.

The corrected outputs in `Claude_Analysis/` supersede `Results and
Analysis/` for any quantitative claim about Q2 or Q3. Q1 (embedding
clustering) and Q4 (compliance) are byte-identical between the two
and either may be cited.
...
- The validation step was run on n=200 (preregistration locked n=50);
  see `deviations.md`. Strictly larger.
- Validation agreement uses partial credit for "mostly" / "partial"
  tags (0.5); see `deviations.md`. Under stricter yes-only scoring,
  `pos_ok` and `rhyme_ok` would fail the 0.85 threshold, downgrading
  six features to exploratory. The sensitivity rerun (in
  `Claude_Analysis/sensitivity_strict_validation/`) shows Q1, Q2, Q3
  conclusions hold under strict scoring.
- Q4 (compliance regression) is degenerate at 0.998 overall
  compliance; reported descriptively per the preregistered fallback.
- The as-run analysis at `Results and Analysis/` had a key-name bug
  affecting Q2/Q3 feature inclusion; corrected outputs are in
  `Claude_Analysis/`. See `Claude_Analysis/deviations.md`.
